            ...
            if (mail != null) {
                ...
                if (user != null) {
                    ...
                    if (user.getPassword().equals(password)) {
                        ...
                        RequestResult result = new RequestResult("Login ok", 0);
                        
                        out.write(new Gson().toJson(result));
                    } else {
                        RequestResult result = new RequestResult("Incorrect password", 0);
                        
                        out.write(new Gson().toJson(result));
                    }

                } else {
                    RequestResult result = new RequestResult("This email is not in the database", -1);
                    out.write(new Gson().toJson(result));
                }

            } else {
                RequestResult result = new RequestResult("An unexpected error has occurred", -2);
                out.write(new Gson().toJson(result));
            }
        } catch (Exception e) {
        }
